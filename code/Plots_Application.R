...
gg1 <- gg1 + theme(axis.text = element_text(size=12), axis.title.x = element_text(size=16), axis.title.y = element_text(size=16),plot.title = element_text(hjust = 0.5,size=16))
...
gg1 <- gg1 + theme(legend.title = element_text(size = 16))
print(gg1)

gg2 <- ggmap(map, extent = "device", legend = "right")
gg2 <- gg2 + geom_point(aes(x = lon, y = lat),colour="#34568B",data=subset(data.df,!ind.fit), alpha = 1, na.rm = T, shape = 16, size = 1.5)
...
gg2 <- gg2 + theme(axis.text = element_text(size=12), axis.title.x = element_text(size=16), axis.title.y = element_text(size=16),plot.title = element_text(hjust = 0.5,size=16))
...
gg2 <- gg2 + theme(legend.title = element_text(size = 16))
print(gg2)

pdf("figure/variogram_contour_3.pdf",width = 4.5*2,height = 5)
#  write R code to align two ggplots
plot_grid(gg1,gg2,nrow=1,rel_widths = c(1.25,1))
dev.off()
...
ext.boxplot <- function(id,n.classes = 50){
  ...
  par(mfrow=c(1,1),mar=c(5,5,2,2),cex.lab=2,mgp=c(2.8,1,0),cex.axis=2)
  plot(sub.dists,sub.extcoef,type="n",pch=20,ylim=c(1,2),xlab="Distance [km]",ylab="Bivariate extremal coefficient")
  boxplot(sub.extcoef~grp.extcoef,add=TRUE,at=classes.mid,xaxt="n",boxwex=15,outline=FALSE)
  lines(extcoef.Pair[,2],extcoef.Pair[,1],lty=2,lwd=4,col="red")
...
pdf(paste0("figure/boxplot_distance_angle.pdf"),width=4.5*4,height=4.5*3)
par(mfrow=c(3,4),mar=c(4,4,3.5,0),pty="s",cex.lab=2,mgp=c(2.8,1,0),cex.main=2,cex.axis=2)
for(i in 1:length(directions)){
    plot(sub.dists.angle[[i]],sub.extcoef[ind.angle[[i]]],xlim=range(sub.dists),type="n",pch=20,ylim=c(1,2),xlab="Distance [km]",ylab="Bivariate extremal coefficient",main=paste0(directions[i],"º"))
...
library(RColorBrewer)
pdf(paste0("figure/boxplot_distance_angle2.pdf"),width=4.5*3,height=4.5*2)
par(mfrow=c(2,3),mar=c(4,4,3.5,0),pty="s",cex.lab=2,mgp=c(2.8,1,0),cex.main=2,cex.axis=2)
